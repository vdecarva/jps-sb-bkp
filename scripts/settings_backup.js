...
/*
 * Improved installation script for SwissBackup add‑on.  It enumerates all running environments
 * and their node groups and builds arrays of options rather than plain objects.  Returning
 * arrays of { value, caption } objects for list fields matches the structure used by the
 * backup‑addon scripts and should ensure that the form renders correctly.
 */
...
resp.infos.forEach(function (envInfo) {
    var env = envInfo.env;
    // Only include environments that are running
    if (env.status == 1) {
        ...
        envInfo.nodes.forEach(function (node) {
            ...
            // Avoid listing duplicate groups within the same environment
            if (!groupsSeen[group]) {
...
// If there are no running environments, return an empty settings object
if (envs.length === 0) {
